...
- [Controlled Taxonomy Vocabulary](#controlled-taxonomy-vocabulary)
...
| `tags` | No | Controlled vocabulary. Every tag must be documented as a term page at `content/tags/<slug>/_index.md`. See [Controlled Taxonomy Vocabulary](#controlled-taxonomy-vocabulary). |
| `categories` | No | Lowercase controlled vocabulary. Every category must be documented as a term page at `content/categories/<slug>/_index.md`. See [Controlled Taxonomy Vocabulary](#controlled-taxonomy-vocabulary). |
...
Tags and categories are a **controlled vocabulary**, not a free-form list. Every
term used in any `tags:` or `categories:` field must have a documented "term
page" at:
...
The `<slug>` is the term name lowercased and converted to kebab-case (e.g.
`systems-thinking`). A term page contains a `title`, `slug`, `description`, and a
short prose explanation. That `description` is shown on the **Tags** and
**Categories** index pages, so each term is described rather than just listed.

This term page is also the **whitelist**: the CI check `npm run lint:taxonomies`
(`scripts/check-taxonomies.sh`) extracts every `tags` and `categories` term referenced in
any non-draft content file under `content/` and fails the build if any term has no
matching term page.

To **introduce a new term**:

1. Create the matching term page (`content/tags/<slug>/_index.md` or
   `content/categories/<slug>/_index.md`) with a `title`, `slug`, `description`,
   and a short body explaining the term.
2. Add the term (`slug`) to the post's front-matter.

To **retire a term**, remove its term page *and* remove the term from every post
that used it; the CI check lists the offending terms and the files that reference
...
| `check-taxonomies.sh` (controlled vocabulary) | All non-draft `tags:` and `categories:` in `content/` vs term pages in `content/tags/` and `content/categories/` | none | `npm run lint:taxonomies` |
...
```bash
# Full lint (markdown + yaml + controlled taxonomy vocabulary)
npm run lint:md && npm run lint:yml && npm run lint:taxonomies
