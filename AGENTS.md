...
| **Lint Taxonomies (controlled vocabulary)** | `npm run lint:taxonomies` | Runs `scripts/check-taxonomies.sh`; fails if a `tags` or `categories` term used in any non-draft content under `content/` lacks a documented term page at `content/<taxonomy>/<slug>/_index.md`. |
| **Lint Hugo Templates** | `hugo --templateMetrics` | Reports unused variables and template performance. |
| **Full Lint + Build** | `npm run lint:md && npm run lint:yml && npm run lint:summary && npm run lint:taxonomies && hugo --minify` | Run all linters and a production build. |
...
- Pipeline stages: `npm run lint:md` → `npm run lint:summary` → `npm run lint:taxonomies` → `yamllint .` → `hugo --minify`.
